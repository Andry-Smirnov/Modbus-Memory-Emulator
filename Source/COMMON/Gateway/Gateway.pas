...
begin
  with GatewayFrm do
  begin
    result := Format(_('%s - %s,%s,%s,%s,%s'),
      [ComPort.Port, BaudRateToStr(ComPort.BaudRate),
      DataBitsToStr(ComPort.DataBits), ParityToStr(ComPort.Parity.Bits),
...
begin
  ...
  begin
    ...
    if ComPort.Connected then
      ComPort.Close;
    if Assigned(GatewayTCPClient) then
      GatewayTCPClient.Free;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if not GatewayTCPClient.Connected then
        GatewayTCPClient.Connect;

      if GatewayTCPClient.Connected then
      begin
        ...
        for I := 0 to CountChar - 3 do
        begin
          ModbusIPBuffer.MBPData[I] := ModbusComBuffer[I];
        end;
        S := SizeOf(ModbusIPBuffer.Header);
        SendIPBuffer := RawToBytes(ModbusIPBuffer, SizeOf(ModbusIPBuffer.Header)
          + CountChar - 2);
