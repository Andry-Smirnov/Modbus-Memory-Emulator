...
type
  TScriptFrm = class(TForm)
    ...
    procedure InterpreterProgramGetValue(Sender: TObject; Identifier: String;
      var Value: Variant; Args: TJvInterpreterArgs; var Done: Boolean);
    procedure FormShow(Sender: TObject);
    procedure SourceEditorChangeStatus(Sender: TObject);
    procedure InterpreterProgramSetValue(Sender: TObject; Identifier: String;
...
begin
  ...
  InterpreterProgram.Adapter.AddGet(EZeroDivide, 'Create', EZeroDivide_Create,
    1, [varEmpty], varEmpty);
...
begin
  InterpreterProgram.Pas := SourceEditor.Lines;
  if InterpreterProgram.Pas.Count <= 1 then
    Exit;
  ...
  try
    try
      ...
      begin
        ...
        StatusPanel.Caption := _('Set: ') +
          IntToStr(V2S(InterpreterProgram.VResult))
      else
      ...
      begin
        modalResult := mrNone;
        StatusPanel.Caption := IntToStr(E.ErrCode) + ': ' +
          ReplaceString(E.Message, #10, ' ');
        if E.ErrPos > -1 then
        begin
...
begin
  ...
  try
    ...
    begin
      modalResult := mrNone;
      MainFrm.Log(IntToStr(E.ErrCode) + ': ' + ReplaceString(E.Message,
        #10, ' '));
    end;
  end;
end;
...
begin
  CaretLabel.Caption := Format(_('X:%d Y:%d'),
    [SourceEditor.CaretX, SourceEditor.CaretY]);
end;
